...
class GitHub(Repository):
    ...
    def post_comment_to_line(self, text, commit_id, file_path, line):
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            raise RepositoryError(f"Erreur avec le commentaire {response.status_code} : {response.text}")
    ...
    def post_comment_general(self, text):
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            raise RepositoryError(f"Erreur avec le commentaire général {response.status_code} : {response.text}")
